...
- The `scraper/` (`crawl`, `scrape`) and `scripts/` (`seed`, `repair-labels`) utilities are optional data-generation tools; they use Playwright and the Gemini API and require `GEMINI_API_KEY`. They are not needed to run or develop the web app.
